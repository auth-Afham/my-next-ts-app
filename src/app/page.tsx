...
// Shape Component
const Shape: React.FC<ShapeProps> = ({ id, shapeType, position, onSelect, isSelected }) => {
  ...
  return (
    ...
    >
      ...
      {shapeType === "cylinder" && <cylinderGeometry args={[0.5, 0.5, 1.5, 32]} />}
...
const SceneContent: React.FC<SceneProps> = ({ addObject, objects, selectedObject, setSelectedObject }) => {
  ...
  return (
    <>
      ...
      {/* Render Shapes */}
      {objects.map((obj) => (
        <Shape key={obj.id} {...obj} onSelect={setSelectedObject} isSelected={selectedObject === obj.id} />
      ))}
...
const ThreeDCanvas: React.FC = () => {
  ...
  const addObject = (point: THREE.Vector3) => {
    ...
    setObjects([...objects, { id: newId, shapeType: shape, position: [point.x, point.y, point.z] }]);
